...
Conceptarium is a research memory system. Its first obligation is to avoid losing conceptual work.
...
Some wording is merely explanatory. Some wording is itself an intellectual artifact.

When a phrase compresses a difficult insight unusually well, preserve it verbatim as a phrase entry or alias even if a more formal term later appears.

Examples of the intended category include phrases such as:

- “Knowledge needs a return address.”
- “Bring your own model.”
- “Subjectification relocates objectivity from the thing to the procedure.”

The formal article can evolve without erasing the phrase.

## 3. Record the birth pressure

A Conceptarium entry should not read like a dictionary descended from heaven.

For important concepts, preserve what was being circled before the term existed:

- the annoyance;
- the unresolved example;
- the contradiction;
- the repeated failed explanation;
- the moment two earlier concepts suddenly connected;
- the question that made ordinary vocabulary feel inadequate.
...
Likewise, a mechanism may be useful while the exact name is provisional.
...
- mark it deprecated if appropriate;
- add `superseded-by`;
- preserve the old definition;
- explain why the new term is better.
...
The canonical entry should contain enough structured and long-form material for each surface to select what it needs.
...
Prefer typed relationships because they preserve why two concepts are connected.

Compare:

> Related: epistemic debt

with:

> Diegetization **pays** or **solves** epistemic debt by supplying an in-world path for otherwise unexplained knowledge.
...
When an example threatens the theory:

- add it;
- narrow the definition if necessary;
- mark an unresolved question;
- record the revision.
...
1. preserve the local term and its provenance;
2. identify the external antecedent;
3. compare the concepts carefully;
4. decide whether they are aliases, overlaps, refinements, or genuinely distinct.
...
Markdown and YAML are the durable source layer.

Everything fancier should be rebuildable from them.
